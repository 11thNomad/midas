...
class CircuitBreaker:
    ...
    def can_trade(self) -> bool:
        ...
        return self.state == BreakerState.NORMAL or self.state == BreakerState.WARNING
    ...
    def update(
        ...
        self.open_position_count = open_positions

        # Reset daily tracking if new day
        if date.today() != self.daily_pnl.date:
            self._reset_daily()

        self.daily_pnl.realized = realized_pnl_today
        self.daily_pnl.unrealized = unrealized_pnl
    ...
    def _check_warning(self):
        """Issue warning when approaching daily limit."""
        ...
        if abs(self.daily_pnl.total) >= warning_threshold and self.daily_pnl.total < 0:
    ...
    def _reset_daily(self):
        ...
        self.daily_pnl = DailyPnL(date=date.today())
        # Only auto-reset if we were in daily trip (not drawdown or killed)
